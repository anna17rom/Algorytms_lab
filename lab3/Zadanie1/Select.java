...
public class Select {
    ...
    private static int[] splitNums = new int[]{5};
        ...
        try {
            while ((line = reader.readLine()) != null) {
                ...
                for (int splitNum : splitNums) {
                    ...
                    recordData(comparisons, swaps, n, k, splitNum);
                    if (n<50){
    ...
    private static void recordData(int comparisons, int swaps, int n, int k, int splitNum) {
        try (FileWriter writer = new FileWriter("Data_SELECT.txt", true)) {
            writer.write(n + " " + k + " " + splitNum + " " + comparisons + " " + swaps + "\n");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
